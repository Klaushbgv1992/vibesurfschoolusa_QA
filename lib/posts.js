...
export async function getAllPosts() {
  try {
    ...
    // Filter out README.md and any other non-blog files
    const blogFiles = fileNames.filter(fileName => {
      // Only include .md files that are not README.md
      return fileName.endsWith('.md') && 
             fileName !== 'README.md' && 
             !fileName.startsWith('.');
    });
    
    const allPosts = await Promise.all(blogFiles.map(async (fileName) => {
